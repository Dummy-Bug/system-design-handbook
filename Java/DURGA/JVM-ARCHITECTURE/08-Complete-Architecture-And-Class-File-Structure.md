...
The class loader has been reading `.class` files for this entire chapter. A class file is not a blob — it is a **fixed sequence of fields in a defined order**, and reading down that sequence it turns out to be the same information the loading phase stores in the method area: the fully qualified name (`this_class`), the immediate parent (`super_class`), the interfaces, the fields, the methods, the constant pool. **That is where those seven items come from.**

Two facts about the file are worth carrying away, and only one of them will ever cost you an evening:

- every `.class` file in existence begins with the same four bytes, **`0xCAFEBABE`** — the magic number, which is how the JVM decides the file is a class file at all
- the two bytes after it are the **version**, and getting that wrong is the single most common deployment failure in Java

The second one is the rest of this section. The first is a nice piece of trivia and nothing more.

> [!question]- Deep dive — the full field-by-field layout of a class file. Worth opening only if you ever touch bytecode tooling like ASM or ByteBuddy; it is not interview material.
> ```java
> class File {
>     Magic_Number;
>     Minor_Version;
...
> [!question]- Deep dive — the magic number in the raw bytes, and what the JVM does when it is wrong. Mechanism; the failure it describes is one you will realistically never meet.
> **The definition, as dictated.** The first **4 bytes** of a class file is the **magic number** — a predefined value used by the JVM to identify a Java class file, that is, whether the `.class` file was generated by a valid compiler or not. **This value should be `0xCAFEBABE`.**
>
> **Seen in the bytes.** Verified on JDK 25 by dumping the first eight bytes of a freshly compiled class:
>
> ```
...
> ```
>
> `cafe babe`, then the minor version `0000`, then the major version `0045` — which is 69 in decimal, and comes up again in the version section below.
>
> **A `.class` file is not readable, and that is the point.** Open one in an editor and you get garbage. The one thing you *could* pick out is the string literals — the text from your `System.out.println` survives in the constant pool. Everything else is binary the JVM reads and you do not.
>
> **What happens when it is wrong.** Whenever we are executing a Java class, if the JVM is unable to find a valid magic number, we get a runtime exception saying **`ClassFormatError: incompatible magic value`**. Demonstrated by patching the first four bytes of a working class to `0xDEADBABE` and running it on JDK 25:
>
> ```
