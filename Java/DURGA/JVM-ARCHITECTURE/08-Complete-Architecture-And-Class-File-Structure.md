...
flowchart TB
    ...
    subgraph JVM["<b>JVM</b>"]
        ...
        subgraph CLS["<b>1 · Class Loader Subsystem</b>"]
            direction LR
            LOAD["<b>Loading</b><br/>bootstrap → extension → application<br/><i>delegation hierarchy</i>"]
...
> [!important] **This is the answer to "explain JVM architecture", and it is worth being able to draw from memory.** The lecture's own claim is that once you have drawn it, there is very little else that can be asked — every follow-up question is a zoom into one box.
...
3. **Loading** — one of the three loaders finds and reads it, following the delegation hierarchy: bootstrap class path first, then extension, then application. Highest priority to bootstrap.
...
| Class path priority | bootstrap → extension → application |
...
> [!info] **Why the heap is singled out.** The lecture's reasoning: Java is an object-oriented language, so almost everything you talk about is objects — and every object is on the heap. The last two areas (PC registers, native method stacks) are, in his words, *"dummy with respect to the programmer"*: real, but never something you act on.
...
>
> **Whenever we try to execute a higher version compiler generated class file with a lower version JVM, we get a runtime exception saying `java.lang.UnsupportedClassVersionError: … (Unsupported major.minor version 51.0)`.**

```mermaid
flowchart LR
    A["compiled with <b>1.6</b><br/>major 50"] -->|"run on <b>1.7</b> JVM"| OK(["✅ works —<br/>newer JVM, older class"])
    B["compiled with <b>1.7</b><br/>major 51"] -->|"run on <b>1.6</b> JVM"| BAD(["❌ UnsupportedClassVersionError<br/>Unsupported major.minor version 51.0"])
```

The lecture demonstrates this with two command prompts side by side — one with its path set to a 1.6 JDK, one on 1.7, colour-coded so you can see which is which. Compile on 1.6 and run on 1.7: fine. Compile on 1.7 and run on 1.6: `UnsupportedClassVersionError`, and the error names `51.0`, which the table above identifies as 1.7.
...
compiled --release 8   : minor 0, major 52
```

> [!warning] **The version table keeps going, and the arithmetic is simple: major version = Java version + 44.** The lecture stops at 1.7 / 51, so extending it:
>
...
>
> The rule and the error are completely unchanged; only the numbers have moved on. If you meet `UnsupportedClassVersionError: … class file version 69.0` in the wild, it means something built on JDK 25 is being run on something older.

> [!info] **`--release N` is how you target an older JVM deliberately.** Compiling with `--release 8` on a JDK 25 produced major version 52 — a class file a Java 8 JVM will accept. This is the supported way to build for an older runtime, and it checks the API you use as well as the bytecode version, which the older `-source`/`-target` flags did not.
