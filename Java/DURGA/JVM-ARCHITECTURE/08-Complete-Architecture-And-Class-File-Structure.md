...
> [!question]- Deep dive — the magic number in the raw bytes, and what the JVM does when it is wrong. Mechanism; the failure it describes is one you will realistically never meet. **The definition, as dictated.** The first **4 bytes** of a class file is the **magic number** — a predefined value used by the JVM to identify a Java class file, that is, whether the `.class` file was generated by a valid compiler or not. **This value should be `0xCAFEBABE`.**
>
