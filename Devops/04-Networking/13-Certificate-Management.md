...
> [!warning] Certificate lifetimes are short, and getting shorter.
...
flowchart TD
    ...
    LATER -->|"yes"| CB
    style CB fill:#1f4f7a,color:#fff
    style LE fill:#7a5a1f,color:#fff
    style NG fill:#1f6f3f,color:#fff
    style LATER fill:#7a5a1f,color:#fff
```
...
> [!tip] The private key never leaves your server.
...
Each component in these notes arrived on its own, to fix one problem. They have never been drawn together, and the order they sit in is not obvious from meeting them one at a time.

```mermaid
flowchart TD
    U["Someone types bookcart.in"] --> DNS["DNS<br/>browser cache, then resolver,<br/>then root, TLD, authoritative"]
    DNS -->|"returns the PUBLIC address —<br/>never an application server's"| RP
    subgraph EDGE["The public edge — one address, port 443"]
        RP["Reverse proxy (nginx)<br/>holds the certificate and private key<br/>terminates TLS, rewrites the port"]
    end
    RP -->|"plain HTTP, inside the network"| GW["API gateway<br/>reads the path, applies<br/>authentication and rate limits"]
    subgraph INTERNAL["Internal network — no address here is reachable from outside"]
        GW -->|"/api/users/*"| LB1["Load balancer<br/>user service"]
        GW -->|"/api/orders/*"| LB2["Load balancer<br/>order service"]
        LB1 --> A1["Replica"]
        LB1 --> A2["Replica"]
        LB2 --> B1["Replica"]
        LB2 --> B2["Replica"]
    ...
    style DNS fill:#7a5a1f,color:#fff
    style RP fill:#1f4f7a,color:#fff
    style GW fill:#1f4f7a,color:#fff
...
The order is the point, and each position is forced rather than chosen.

**The reverse proxy is outermost** because it is the only component holding the private key, so nothing inside it can read an encrypted request. Decryption has to happen before anything else can make a decision.

**The gateway comes next** because deciding which service a request belongs to requires reading its path, and the path is only readable once TLS has been undone.

**The load balancers come last** because each one chooses between interchangeable copies of a single service. That choice only makes sense after the service has already been identified.

Not every system has all four. A single application with no microservices has no gateway; a small deployment on one machine has no load balancer. What does not vary is the order, because each step depends on the one before it having already happened.

> [!note] Two roles frequently sit in the same process.
> The reverse proxy, the load balancer and the gateway are jobs, not products, and nginx can do all three. Drawn as separate boxes they are easier to reason about; deployed, you may well find one nginx configuration performing two of them at once. The order the work happens in is unchanged either way.

Read end to end, a single request's journey is this. A name is typed. DNS resolves it through browser cache, resolver and hierarchy to an address — the address of a reverse proxy or a load balancer, never of an application server. TCP establishes a connection. TLS negotiates a version and an algorithm, the server proves its identity with a certificate signed by an authority the client already trusts, and both sides derive a shared secret neither transmitted. The reverse proxy decrypts what arrives, rewrites the port, and hands it inward. A gateway may route it by path to the right service, and a load balancer picks which replica of that service answers. The application, at the end of all of it, receives a plain HTTP request on port 8080 and has no idea any of the rest happened.
