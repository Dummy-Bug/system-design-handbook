...
| DP » String | ~4 | ◐ 1/2 (#25, 2026-06-23) — 2nd rep rolls cross-band (no clean in-band supply) |
...
| **Monotonic Stack** | ~2 | ★ blind-spot, carried 1/2 — scarce. **#19 ramp + #27 car-fleet both MIS-PICKS** (solved by sort / running-max, stack not load-bearing → no credit). 2nd rep owed on carried #9 max-chunks (or Asteroid-Collision / Car-Fleet-II cross-band) |
...
| Two Pointers | 0/2 | **2** | 32 soft-fail, 35 soft-hinted, 38 hard-fail at 1550-1600 (none counted). #03 push-dominoes (over-modeled) + #17 advantage-shuffle (TreeMap-greedy) + #24 move-pieces (WA+reframe) + **#29 expressive-words (2026-06-25, WA-then-AC, 44m → NO REP; [incomplete-validation], same "both endpoints must exhaust" miss as #24)** all spent with NO credit. **All 4 queue/in-band two-ptr shots now exhausted → needs 2 FRESH non-queue picks.** Still 0 clean self-derived. | |
...
| Stack (plain) | 0/2 | **2** | 30 hard-fail at 1550-1600. **All THREE Stack-tagged shots spent with NO rep:** #11 min-swaps-balanced (editorial hard-fail) + #16 minimum-remove (2026-06-15 RE-SOLVE — LC 1249 pre-solved 2026-06-03 in stack-reflex track) + **#30 di-string (2026-06-26 — MIS-PICK: honest optimal is greedy block-reversal, stack incidental; also rule-taught + WA-then-AC).** → **needs 2 FRESH non-queue Stack picks** where the stack is genuinely load-bearing. |
| ~~Backtracking / Subset-Enum~~ | **2/2** ✅ | 0 | **OWNED 2026-06-11** (#05) — moved to owned table above |
| DP » Linear | 1/2 | **1** | #23 knight-dialer (2026-06-23) clean rep 1; 28,36 soft-fail at 1550-1600. **#27 count-ways-build-good-strings (2026-06-24, WA-then-AC + debug help → NO REP)** — bugs: stale-memo read of unvisited `dp[low-1]` + mod-underflow on `high-low` subtraction; overflow proof banked (≥exponential ⇒ mod every step). Still owes 1 clean. |
...
| DP » String | 1/2 | **1** | **#25 number-of-ways-to-select-buildings (2026-06-23, CLEAN first-sub self-derived, subsequence-count DP) → rep 1.** Prior: #04 partition-≤k (Greedy ride-along) + #22 flip-string-to-monotone (editorial — NO REP). **2nd rep has no clean in-band supply → rolls cross-band (rule 6B).** |
...
| **Monotonic Stack ★** | 1/2 (22) | **1** | **TWO mis-picks now:** #19 max-width-ramp (2026-06-15, solved via sort) + **#27 car-fleet (2026-06-24, running-max scan + Socratic help)** — both stack-collapses-to-top, neither credits [[lc-index-bookkeeping-overmodel]]. Stays 1/2. **Reflex-gap fixed earlier:** new **Stack Atom 09** (farthest/widest). **Rep 2 owed cold on carried #9 max-chunks** (or Asteroid-Collision / Car-Fleet-II cross-band) where the stack is genuinely load-bearing and Atom 09 must fire unaided. |
