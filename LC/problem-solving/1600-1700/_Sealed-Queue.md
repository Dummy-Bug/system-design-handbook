...
25. [x] ~~https://leetcode.com/problems/move-pieces-to-obtain-a-string/~~ ⚠️❌ dealt 2026-06-23 (**WA-then-AC soft fail + reframe given before solve → NO REP**; **Two-Pointers stays 0/2**. WA-cause [incomplete-validation]: v1 matched target→start but never checked every START piece got consumed → dangling `L` passed; v2 trailing-blank sweep fixed it. Meta: user skipped the natural queue solution "because Queue isn't a bucket" → blind-deal leak [[lc-buckets-are-accounting-not-solving]]. clean-rate 16/22) → `First-Attempt/24`
26. [x] ~~https://leetcode.com/problems/number-of-ways-to-select-buildings/~~ ✅ dealt+AC 2026-06-23 (**CLEAN first-sub self-derived AC**; pick/non-pick subsequence-count DP; Run-only sample fails, no WA submit, no hint. **DP » String 0/2 → 1/2.** Banked memo-key reflex: key omitted `i`, cached on derived `building` → collisions; key must = exact args the answer depends on. clean-rate 17/23) → `First-Attempt/25`
27. [x] ~~https://leetcode.com/problems/car-fleet/~~ ❌ dealt 2026-06-24 (**Socratic walk-through → NO REP**; stuck >30m on a vague merge-intervals idea, approach led Socratically. **MONO-STACK MIS-PICK** — honest solution is a running-max scan (1 var), stack collapses to its top → even a clean solo AC wouldn't credit; same disease as #19 ramp [[lc-index-bookkeeping-overmodel]]. **Mono-Stack stays 1/2.** Float-precision lesson banked: min distinct-arrival gap `1/(s₁s₂)≈10⁻¹²` < float res → use long cross-mult. clean-rate unchanged 17/23 [guided, not counted]) → `First-Attempt/26`
28. [x] ~~https://leetcode.com/problems/count-ways-to-build-good-strings/~~ ⚠️❌ dealt 2026-06-24 (**WA-then-AC soft fail + debugging help → NO REP**; **DP-Linear stays 1/2**). 2 bugs: (1) `[stale-memo]` read `dp[low-1]` directly but recursion stepping −2/−4 from 5 never visits index 4 → stale `-1`; must INVOKE `helper(low-1)`. (2) `[mod-underflow]` reduced `high` can drop below reduced `low` (mod not order-preserving) → `((a-b)%MOD+MOD)%MOD`. Overflow proof banked: recurrence ≥ exponential (2^n / φ^n), crosses long by len ~63–90 « 10^5 ⇒ mod every step. clean-rate 17/24) → `First-Attempt/27`
29. [x] ~~https://leetcode.com/problems/expressive-words/~~ ⚠️❌ dealt 2026-06-25 (**WA-then-AC soft fail, 44m OVER-CAP → NO REP**; **Two-Pointers stays 0/2**). WA-cause [incomplete-validation]: accept test was `sb.equals(s)` only — checked s-side built but not word-side consumed; on `s="heeellooo",word="heeelloooworld"` loop exits when `i` hits `n1` while `j` still has `"world"` → counted (exp 0). Fix: add `j == n2` (both endpoints must exhaust). **Same recurring miss as #24 move-pieces.** clean-rate 17/25) → `First-Attempt/28`
30. [x] ~~https://leetcode.com/problems/construct-smallest-number-from-di-string/~~ ❌ dealt 2026-06-25, solved 2026-06-26 (**rule taught Socratically + WA-then-AC → NO REP**; **Stack stays 0/2**). WA-cause [stale-default]: trailing flush `int j=0; … reverse(ans,j,n)` fired on an empty stack when pattern ends in `I` → spurious full-array reverse (`"DI"`→`312`, exp `213`); fix guard `j=-1`. **MIS-PICK** — honest optimal is greedy block-reversal (reverse each maximal D-run on ascending base), stack incidental → wouldn't credit Stack even if clean [[lc-buckets-are-accounting-not-solving]]. Stack (0/2) needs 2 FRESH non-queue picks. clean-rate 17/26) → `First-Attempt/29`
