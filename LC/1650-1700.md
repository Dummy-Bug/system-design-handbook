...
class Solution {
    ...
    private int getPrefixLength(String w1, String w2) {
        ...
        for (int i = 0; i < n; i++) {
            if (w1.charAt(i) != w2.charAt(i)) return i;
        }
        return n;
    }
    ...
    public int[] longestCommonPrefix(String[] words) {
        ...
        int[] right = new int[n];
        for (int i = n - 2; i >= 0; i--) {
            int prefixLength = getPrefixLength(words[i], words[i + 1]);
        ...
        for (int i = 0; i < n; i++) {
            ...
            int rightMax = (i != n - 1) ? right[i + 1] : 0;
            int maxPrefix = Math.max(leftMax, rightMax);
            if (i != 0 && i != n - 1) {
                maxPrefix = Math.max(maxPrefix, getPrefixLength(words[i - 1], words[i + 1]));
            }
            answer[i] = maxPrefix;
        }
        return answer;
...
O(n × L) time where L is average word length, O(n) space. Sum constraint ensures total character work stays within 10^5.
