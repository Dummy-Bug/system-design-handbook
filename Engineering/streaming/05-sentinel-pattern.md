...
> Prerequisite: [[driver-pattern]] — establishes the driver / consumer split this note builds on.
...
## Why `finally` and Not the Bottom of the Function

Putting the sentinel push at the bottom of the function only catches the happy path. Any early `return` or raised exception skips straight past it. `finally` does not skip — that's the whole point of putting cleanup there.

> [!info] The sentinel is non-optional cleanup. Closing the consumer's HTTP response is not something you do "if everything went well." It's something that must happen on every exit path. That requirement maps directly onto what `finally` is for.
