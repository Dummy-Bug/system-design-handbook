The third and final stage from [[06-The-Three-Stages]]. After [[11-Supervised-Fine-Tuning]] the model answers like an assistant; **reinforcement learning** pushes it one step further.
...
That is a genuine economic difference from [[11-Supervised-Fine-Tuning]], where every training example had to be written by a person.
...
This is where reasoning ability actually comes from, and it connects directly to the chain-of-thought material flagged at the end of [[11-Supervised-Fine-Tuning]].
...
>
> The nuance: **other companies were already doing it.** DeepSeek R1 was not necessarily first to the method — it was first to put the whole thing together **publicly**. The paper gets its own treatment later in the course.
...
**It guarantees** that the model is optimised toward outputs humans actually prefer, rather than outputs that merely continue text plausibly.

**It does not guarantee truth.** The reward model predicts *human rankings*, and humans reward answers that read well. An answer that is confident, well-structured and wrong can score higher than one that is hesitant and right.

**It inherits every bias in the ranking.** Whatever the human rankers preferred — length, tone, certainty — becomes what the model optimises for, at scale, permanently.
