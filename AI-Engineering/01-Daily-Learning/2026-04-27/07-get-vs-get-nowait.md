...
`asyncio.Queue` gives the consumer two ways to read an item:
...
The whole difference comes down to **what happens when the queue is empty**. This note walks through the scenario, explains why this side is *not* symmetric with the producer side, and then maps each method to the situations where it actually fits.

---

## The Setup

The queue exists. No producer has put anything in yet — or the consumer drained the last item already. The queue is **empty**.

The consumer tries to read. What can the queue do?

1. Wait until something arrives.
2. Raise an error right now.
3. Return some sentinel like `None`.

asyncio refused option 3 — silent sentinels force every caller to check `if item is None:` and are a notorious bug source. So you get the other two as **two separate methods**, and you pick which behavior you want at the call site:

| Behavior you want | Method | If empty |
|---|---|---|
| Wait until something arrives | `await queue.get()` | Suspends the consumer until a producer puts something |
| Fail loudly right now | `queue.get_nowait()` | Raises `asyncio.QueueEmpty` immediately |

Same shape as `put` / `put_nowait`. The "wait" version pauses your coroutine; the "nowait" version refuses to pause and raises instead.
