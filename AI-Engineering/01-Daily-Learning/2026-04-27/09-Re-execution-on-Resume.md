...
You know LangGraph re-executes the interrupted node from line 1 on resume (note 09). That sounds alarming. In practice it's mostly fine — but only if you understand exactly what fine means and where it stops being fine.
...
On the resume run, this line does **not** pause. It returns the human's selected value directly. This is the key mechanic — covered in detail in note 11.
