...
`asyncio.Queue` gives the producer two ways to push an item:
...
The whole difference between them comes down to **what happens when the queue is full**. Walk through that scenario step by step and the entire API makes sense.

---

## The Setup

You created a bounded queue:

```python
queue = asyncio.Queue(maxsize=3)
```

A producer has already pushed 3 items. The consumer hasn't drained anything. The queue is **full**.

Now the producer tries to push a 4th item. What can the queue do?

In principle there are three options:

1. Wait until there's space.
2. Raise an error right now.
3. Silently drop the new item.

asyncio refused to do option 3 — silent loss is almost always a bug. So it gave you the other two as **two separate methods**, and you pick which behavior you want at the call site:

| Behavior you want | Method | If full |
|---|---|---|
| Wait until there's space | `await queue.put(item)` | Suspends the producer until a slot frees up |
| Fail loudly right now | `queue.put_nowait(item)` | Raises `asyncio.QueueFull` immediately |
