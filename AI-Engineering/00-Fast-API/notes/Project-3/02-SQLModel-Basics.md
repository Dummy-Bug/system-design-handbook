...
- **Is a Pydantic model** — constructing one validates its fields exactly like `BaseModel` always has, because underneath, it genuinely is one of those too.
...
- **`table=True`** — this class becomes an actual **database table**. Every field becomes a column. Rows in the table correspond to instances of this class.
- **No `table=True`** — the class behaves like ordinary Pydantic: a validation shape, nothing persisted. This is the same `BaseModel`-style behavior seen in every earlier project.
